...
class TestRealWorldIntegration:
    ...
    def test_sentiment_analysis_on_real_reddit_data(self):
        ...
        for article in reddit_articles:
            try:
                ...
                logger.warning(
                    f"VADER analysis failed for article {article['id']}: {e}"
                )
        ...
        for result in results:
            assert (
                -1.0 <= result["vader_score"] <= 1.0
            ), f"Invalid sentiment score: {result['vader_score']}"
            assert result["vader_label"] in [
                "Positive",
                "Negative",
                "Neutral",
            ], f"Invalid sentiment label: {result['vader_label']}"
    ...
    def test_llm_sentiment_analysis_on_real_data(self):
        ...
        for result in results:
            assert (
                -1.0 <= result["llm_score"] <= 1.0
            ), f"Invalid LLM sentiment score: {result['llm_score']}"
            assert result["llm_label"] in [
                "Positive",
                "Negative",
                "Neutral",
            ], f"Invalid LLM sentiment label: {result['llm_label']}"
    ...
    def test_sentiment_analysis_performance_on_real_data(self):
        ...
        assert (
            vader_time < 1.0
        ), f"VADER performance too slow: {vader_time:.2f}s for {len(reddit_articles)} articles"

        # Average time per article should be reasonable
        avg_time_per_article = vader_time / len(reddit_articles)
        assert (
            avg_time_per_article < 0.1
        ), f"VADER too slow per article: {avg_time_per_article:.3f}s"
    ...
    def test_parallel_sentiment_analysis(self):
        ...
        def analyze_article(article: dict[str, Any]) -> dict[str, Any]:
            ...
            try:
                ...
                return {"article_id": article["id"], "error": str(e), "success": False}
    ...
    def test_subreddit_sentiment_analysis(self):
        ...
        for subreddit, count, avg_sentiment in subreddit_stats:
            if avg_sentiment is not None:
                assert (
                    -1.0 <= avg_sentiment <= 1.0
                ), f"Invalid average sentiment for r/{subreddit}: {avg_sentiment}"
    ...
    def test_end_to_end_pipeline_on_real_data(self):
        ...
        sentiment_score = sentiment_service.analyze_sentiment(
            article.text or article.title or ""
        )
        ...
        assert (
            -1.0 <= sentiment_score <= 1.0
        ), f"Invalid sentiment score: {sentiment_score}"
        assert sentiment_label in [
            "Positive",
            "Negative",
            "Neutral",
        ], f"Invalid sentiment label: {sentiment_label}"
        ...
        for link in ticker_links:
            ...
            assert hasattr(
                link, "confidence"
            ), "TickerLink should have confidence attribute"
            assert (
                0.0 <= link.confidence <= 1.0
            ), f"Invalid confidence: {link.confidence}"

        logger.info(
            f"Pipeline completed: {len(ticker_links)} ticker links, sentiment: {sentiment_label} ({sentiment_score:.3f})"
        )
