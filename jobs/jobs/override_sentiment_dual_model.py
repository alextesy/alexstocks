...
def get_articles_for_dual_model_override(
    ...
    if force_all:
        ...
        query = select(Article).where(Article.sentiment.is_not(None))
        logger.info(
            "Selecting articles with existing sentiment for dual model re-analysis"
        )
...
def analyze_single_article_dual_model(article: Article) -> tuple[int, float | None]:
    ...
    try:
        ...
        logger.debug(
            f"Dual model sentiment for {article.source} {article.id}: {sentiment_score:.3f}"
        )
        return article.id, sentiment_score

    except Exception as e:
        logger.warning(
            f"Failed dual model sentiment analysis for article {article.id}: {e}"
        )
        return article.id, None
...
def override_articles_parallel(
    ...
    logger.info(
        f"Overriding sentiment for {len(articles)} articles with dual model using {max_workers} parallel workers"
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        with tqdm(
            total=len(articles), desc="Dual model sentiment analysis", unit="articles"
        ) as pbar:
    ...
    try:
        ...
        with tqdm(
            total=len(sentiment_results), desc="Database updates", unit="articles"
        ) as pbar:
            # Process in batches
                ...
                for article_id, sentiment_score in batch:
                    if sentiment_score is not None:
                        try:
                            ...
                            logger.warning(
                                f"Failed to update article {article_id}: {e}"
                            )
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, help="Maximum number of articles to process"
    )
    parser.add_argument("--source", type=str, help="Filter by source (e.g., 'reddit')")
    parser.add_argument(
        "--hours-back", type=int, help="Only process articles from the last N hours"
    )
    parser.add_argument(
        "--force-all",
        action="store_true",
    ...
    logger.info(
        f"Configuration: limit={args.limit}, source={args.source}, "
    ...
    try:
        ...
        successful = override_articles_parallel(
            articles=articles, max_workers=args.max_workers, batch_size=args.batch_size
        )

        logger.info(
            f"Dual model sentiment override complete: {successful}/{len(articles)} articles processed successfully"
        )
