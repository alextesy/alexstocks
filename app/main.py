...
async def get_stock_data(symbol: str):
    ...
    try:
        ...
        if stock_data:
            ...
            return JSONResponse(
                status_code=404,
                content={"error": f"Stock data not found for {symbol}"}
            )
    except Exception as e:
        ...
        try:
            ...
            for article, confidence, matched_terms in articles_with_confidence:
                if article.source == 'reddit_comment':
                    title = article.text[:100] + "..." if len(article.text) > 100 else article.text
        ...
        try:
            ...
            if historical_data:
                ...
                if current_price:
                    ...
                    try:
                        chart_data = await stock_service.get_chart_data_for_ui(symbol.upper(), period)
                        if chart_data:
                            return chart_data
                    except Exception as e:
...
async def home(request: Request, page: int = 1) -> HTMLResponse:
    ...
    try:
        ...
        tickers_query = (
            ...
            .outerjoin(
                ...
                and_(
                    ...
                    ArticleTicker.article_id.in_(
                        db.query(Article.id).filter(Article.published_at >= twenty_four_hours_ago)
                    )
                )
            )
            ...
            .group_by(
                Ticker.symbol, Ticker.name, top_tickers_subquery.c.recent_article_count,
                StockPrice.price, StockPrice.previous_close,
                StockPrice.change, StockPrice.change_percent, StockPrice.market_state,
                StockPrice.currency, StockPrice.exchange, StockPrice.updated_at
            )
...
async def get_all_tickers(
    ...
    try:
        ...
        tickers_query = (
            base_query
            .add_columns(func.coalesce(recent_count.c.recent_count, 0).label('recent_activity_count'))
            .outerjoin(ArticleTicker, Ticker.symbol == ArticleTicker.ticker)
            .outerjoin(Article, ArticleTicker.article_id == Article.id)
            .outerjoin(StockPrice, Ticker.symbol == StockPrice.symbol)
            .outerjoin(recent_count, Ticker.symbol == recent_count.c.ticker)
            .group_by(
                Ticker.symbol, Ticker.name, StockPrice.price, StockPrice.previous_close,
                StockPrice.change, StockPrice.change_percent, StockPrice.market_state,
    ...
    try:
        ...
        if historical_data:
            ...
            if stock_price:
                ...
                try:
                    chart_data = await stock_service.get_chart_data_for_ui(ticker.upper(), "1mo")
                except Exception as e:
        ...
        for article, confidence, matched_terms in articles_with_confidence:
            ...
            article_dict = {
                ...
                "author": author_info,
                "subreddit": subreddit_info,
                "full_text": article.text if article.source == 'reddit_comment' else None,
                "matched_terms": matched_terms or [],
            }
            articles.append(article_dict)
