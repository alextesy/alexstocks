...
def test_relink_sample(sample_size: int = 20):
    ...
    try:
        ...
        sp500_tickers = service.db.query(Ticker).filter(Ticker.is_sp500 == True).count()
        ...
        if articles_with_improvements > 0:
            ...
            print(f"   Recommend running full re-linking on all articles.")
        else:
            print(f"\n🤔 No improvements detected in sample.")
            print(f"   May want to investigate ticker linking logic.")
