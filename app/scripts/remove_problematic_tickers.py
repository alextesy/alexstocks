...
from app.db.models import Ticker, ArticleTicker
...
KEEP_TICKERS: Set[str] = {
    ...
    'IREN', 'BITF', 'NBIS', 'EOD', 'WWW', 'BRO', 'PRE', 'TRUE', 'TECH'
}

# Problematic tickers that are common English words
PROBLEMATIC_TICKERS: Set[str] = {
    'DON', 'PUMP', 'YEAR', 'WANT', 'WAY', 'WELL', 'SAY', 'REAL', 'COM', 'PORT', 'EVER', 'AGO',
    'PLAY', 'LOVE', 'HIT', 'MAN', 'SHE', 'NICE', 'HOPE', 'JOB', 'CASH', 'LOT', 'SUB', 'PAY'
}
...
def remove_problematic_tickers(dry_run: bool = True) -> None:
    ...
    if dry_run:
        ...
        for symbol in KEEP_TICKERS:
            ...
            if ticker:
                article_count = db.query(ArticleTicker).filter(ArticleTicker.ticker == symbol).count()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Remove problematic tickers that are common English words")
