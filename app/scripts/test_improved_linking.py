...
def test_improved_linking():
    ...
    try:
        ...
        if visa_link:
            print(
                f"✓ Visa Inc article correctly linked to V with {visa_link.confidence:.2f} confidence"
            )
        else:
        ...
        visa_app_link = next(
            (link for link in visa_app_links if link.ticker == "V"), None
        )
        if visa_app_link:
            print(
                f"✗ Visa application article incorrectly linked to V with {visa_app_link.confidence:.2f} confidence"
            )
        else:
        ...
        if apple_link:
            print(
                f"✓ Apple article correctly linked to AAPL with {apple_link.confidence:.2f} confidence"
            )
        else:
        ...
        ambiguous_link = next(
            (link for link in ambiguous_links if link.ticker == "V"), None
        )
        if ambiguous_link:
            print(
                f"✗ Ambiguous article incorrectly linked to V with {ambiguous_link.confidence:.2f} confidence"
            )
        else:
