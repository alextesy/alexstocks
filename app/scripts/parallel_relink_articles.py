...
def process_article_batch(batch_data: Tuple[List[dict], List[dict], int]) -> dict:
    ...
    try:
        ...
        ticker_linker = TickerLinker(tickers, max_scraping_workers=2)  # Reduce per-worker threads
        ...
        with tqdm(
            ...
            for article_data in article_data_list:
                try:
                    ...
                    ticker_links = ticker_linker.link_article(article, use_title_only=True)
...
class ParallelArticleRelinkingService:
    ...
    def get_articles_batch_data(self, limit: int = None, batch_size: int = 50) -> List[List[dict]]:
        """Get article data in batches for parallel processing."""
        logger.info(f"Loading articles for parallel processing...")
    ...
    def parallel_relink_articles(self, limit: int = None, batch_size: int = 50) -> dict:
        ...
        print(f"✅ Ready to process {total_articles:,} articles in {len(article_batches)} batches")
            ...
            with tqdm(
                ...
                for future in as_completed(future_to_batch):
                    ...
                    try:
                        ...
                        # Aggregate stats
                        self.stats['articles_processed'] += worker_result['articles_processed']
                        self.stats['articles_with_links'] += worker_result['articles_with_links']
                        self.stats['total_links_removed'] += worker_result['total_links_removed']
                        self.stats['total_links_added'] += worker_result['total_links_added']
    ...
    def print_summary(self):
        ...
        if self.stats['articles_processed'] > 0:
            coverage_rate = (self.stats['articles_with_links'] / self.stats['articles_processed']) * 100
            avg_links_per_article = self.stats['total_links_added'] / self.stats['articles_processed']
...
def main():
    ...
    try:
        ...
        if not auto_confirm:
            ...
            print(f"Usage: python parallel_relink_articles.py --yes [--limit=N] [--workers=N] [--batch-size=N]")
            return
        ...
        result = service.parallel_relink_articles(
            limit=limit,
