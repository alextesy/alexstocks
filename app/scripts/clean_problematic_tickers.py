...
ENGLISH_STOP_WORDS = {
    ...
    "LITTLE",
    "FEW",
    "SEVERAL",  # Action words that could appear in financial context
    "GET",
    "GOT",
    "GIVE",
    "TAKE",
...
class TickerCleaner:
    ...
    def identify_problematic_tickers(self) -> list[str]:
        ...
        for (symbol,) in all_tickers:
            ...
            if (
                len(symbol) == 1
                and symbol_upper in AMBIGUOUS_SINGLE_LETTERS
                and symbol_upper not in IMPORTANT_SINGLE_LETTERS
            ):
                problematic.append(symbol)
                continue
    ...
    def remove_problematic_tickers(
        self, symbols_to_remove: list[str], confirm: bool = True
    ) -> dict:
    ...
    def print_analysis(self, problematic_tickers: list[str], usage_stats: dict):
        ...
        two_letters = [
            s
            for s in problematic_tickers
            if len(s) == 2 and s.upper() in COMMON_TWO_LETTER
        ]
        ...
        total_links = sum(
            usage_stats.get(s, {}).get("link_count", 0) for s in problematic_tickers
        )
        sp500_count = sum(
            1
            for s in problematic_tickers
            if usage_stats.get(s, {}).get("is_sp500", False)
        )
...
def main():
    ...
    try:
        ...
        if remove_flag:
            ...
            result = cleaner.remove_problematic_tickers(
                problematic_tickers, confirm=False
            )
